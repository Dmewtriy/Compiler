...
namespace Compiler.Controllers
{
    public class MainController
    {
        ...
        {
            // 1. Настраиваем парсер Markdown (включаем расширения для таблиц и т.д.)
            var pipeline = new MarkdownPipelineBuilder().UseAdvancedExtensions().Build();

            // 2. Конвертируем MD в HTML
            string htmlBody = Markdown.ToHtml(markdownContent, pipeline);

            // 3. Добавляем немного CSS, чтобы текст в WebBrowser выглядел современно
            string finalHtml = $@"
            <html>
            <head>
